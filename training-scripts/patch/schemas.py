...
class OpenAIFunctionPropertySchema(BaseModel):
    ...
    def resolve_type_from_anyof(self):
        """If type is missing but anyOf is present, derive a type string from it."""
        if self.type is None and self.anyOf:
            non_null_types = [entry["type"] for entry in self.anyOf if entry.get("type") != "null"]
            self.type = non_null_types[0] if non_null_types else "string"
