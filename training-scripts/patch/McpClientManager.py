        ...
        for client in self.clients:
            try:
                ...
                if e.response.status_code == 502:
                    logger.warning(f"MCP server returned 502. Skipping client: {client}")
                    continue
                raise

            except httpx.HTTPError as e:
                logger.warning(f"MCP HTTP error from client {client}: {e}")
                continue

            except Exception as e:
                logger.warning(f"Unexpected MCP error from client {client}: {e}")
                continue
